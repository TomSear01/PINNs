...
#vel_model = torch.load('velocity_Gaussian.pth',map_location=torch.device('cpu')) #Load previously trained velocity NN
vel_model.to(device)
vel_model.eval()
...
plotTraining(x,y,v_plot,np.array([[0,0],[0,0]]),np.array([[0,0],[0,0]]))
...
for i in range(100): # reducing learning rate after each
    print(f"\nLoop: {i+1}, lr:{lr}")
